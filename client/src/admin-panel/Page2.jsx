  ...
  <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        />
        {errors.familyNumber && (
          <p className="text-red-500 text-sm">{errors.familyNumber.message}</p>
        )}
      </div>
    </div>
    ...
    {/* Row 5: Flour Preference and Feedback */}
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <label className="block text-sm font-medium text-gray-700">Flour Preference</label>
        <select
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700">Valuable Feedback for Customer</label>
        <textarea
          {...register("feedback")}
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
        ></textarea>
    ...
    {/* Row 6: Report Completed By */}
    <div>
      <label className="block text-sm font-medium text-gray-700">Report Completed By</label>
      <input
        type="text"
        {...register("reportCompletedBy", { required: "Report Completed By is required" })}
        className="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
      />
      {errors.reportCompletedBy && (
        <p className="text-red-500 text-sm">{errors.reportCompletedBy.message}</p>
      )}
    </div>
    ...
    <div className="text-center">
      ...
      >
        Submit Survey{' '}
        {loading ? (
          <Spinner className="ml-2" aria-label="Default status example" />
        ) : null}
      </button>
    </div>
  </form>
